    ...
    VALUES ( @orderid, @customId, @designId, Now(), @tier, @cover, @suborderId";
        ...
        private async Task<string> GetUserIdByAllUsername(string username) {
            using(var connection = new MySqlConnection(connectionstring)) {
                ...
                // Step 1: Get the UserId regardless of email confirmation status
                string sql = "SELECT Id, EmailConfirmed FROM aspnetusers WHERE UserName = @username";
                ...
                using(var command = new MySqlCommand(sql,connection)) {
                    ...
                    using(var reader = await command.ExecuteReaderAsync()) {
                        if(await reader.ReadAsync()) {
                            ...
                            bool isEmailConfirmed = reader.GetBoolean("EmailConfirmed");

                            if(!isEmailConfirmed) {
                                throw new InvalidOperationException("Email is not verified for this user.");
                            }
